for (let i = 0; i < allCells.length; i++) {
    ...
    allCells[i].addEventListener("blur", function () {
      ...
      if (cellObject.value == data) {
        //  agar UI and DB ki value same h toh return karjao
        return;
      }
      if (cellObject.formula) {
        //agar cell ka formula pada  h DB mai toh  remove kardo formula ko and parent ke array mai jaa kar children remove kardo
  ...
  formulaBar.addEventListener("keydown", function (e) {
    //yeh tab chalega jabb user formula bar mai formula type kar karke "enter" press kare
    if (e.key == "Enter" && formulaBar.value) {
      ...
      if (currentFormula != cellObject.formula) {
        //agar phele wala formula ,current formula ke equal nhi h toh(i.e user ne formula update kar dia toh)
  ...
  function setCell(value, formula) {
    //this func sets value in UI  and  sets value and formula in sheetDB
  ...
  function findUICellElement() {
    // dom ke element ka reference aa rha h jo address bar mai h
    let address = addressInput.value;
  ...
  function getRIDCIDfromAddress(address) {
    //humare string form wale address de rhe h aur yeh func rid and cid ke indexes de rha h wapas
  ...
  function updateChildren(cellObject) {
    //(recursive func)Children se apne aap ko evaluate karao aur fir apne children ko evaluate karwao
  ...
  function SetChildrenCell(value, formula, rid, cid) {
    //this func sets value and formula of children cells in UI and sheetDB both
  ...
  function removeFormula(cellObject, myName) {
    //(just opposite function of "setParentCHArray")
